...
export default function Hero() {
  ...
  return (
    ...
    paddingTop: "clamp(5.5rem, 18vw, 9rem)",
      ...
      {/* Gold ambient glow — right side, % width so it never escapes overflow-hidden */}
      ...
      {/* Left blue depth glow — no negative offset, stays inside container */}
      ...
      <div className="relative z-10 layout-container
        ...
        <div className="flex flex-col w-full max-w-[600px]" style={{ gap: "var(--igap-sm)" }}>
          ...
          <p ref={subRef} style={{ opacity: 0 }}
            className="text-white/70 text-sm sm:text-base lg:text-lg leading-relaxed font-normal">
            {t.hero.classes}
          </p>
          ...
          {/* ── CTAs ── */}
          <div ref={ctasRef} className="cta-stack">
          ...
          {/* ── Primary phone ── */}
          <div ref={phoneRef} style={{ opacity: 0 }} className="flex flex-wrap items-center gap-2 sm:gap-3">
      ...
      <motion.div
        ...
        <motion.div
          ...
          className="text-gold/50 text-xl leading-none">↓</motion.div>
      </motion.div>
    </div>
  );
}
