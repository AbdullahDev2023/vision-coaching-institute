...
function StatCard({
  ...
  return (
    <div className="relative group text-center p-4 sm:p-6 rounded-2xl overflow-hidden cursor-default"
...
export default function About() {
  ...
  useScrollReveal(leftRef  as React.RefObject<HTMLElement>, { y: 30, x: 0, duration: 0.7, start: "top bottom" });
  useScrollReveal(rightRef as React.RefObject<HTMLElement>, { y: 30, x: 0, duration: 0.7, delay: 0.12, start: "top bottom" });

  return (
    <div ref={sectionRef} className="relative section-pad overflow-clip"
      ...
      <div className="relative z-10 layout-container section-inner">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 items-start" style={{ gap: "var(--igap)" }}>
            ...
            <ul className="item-gap">
              {t.about.features.map((item) => (
                <li key={item} className="flex items-start gap-4">
                  <span className="mt-1 w-6 h-6 rounded-full flex items-center justify-center flex-shrink-0"
          ...
          <div ref={rightRef} style={{ opacity: 0 }} className="flex flex-col item-gap">
            <div className="grid grid-cols-3" style={{ gap: "var(--igap-sm)" }}>
            ...
            {/* Vision card */}
            <div className="rounded-2xl p-6 sm:p-8 border border-primary/10 relative overflow-hidden"
              style={{ background: "linear-gradient(135deg, #0A1F5C 0%, #1A3A8F 100%)" }}>
