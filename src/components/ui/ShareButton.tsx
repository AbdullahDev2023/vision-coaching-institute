...
export default function ShareButton({ href, label = "Share", className = "" }: ShareButtonProps) {
  return (
    <a
      ...
      aria-label={`Share on WhatsApp: ${label}`}
      className={`inline-flex items-center gap-1.5 px-3 py-1.5 rounded-full text-[11px] font-bold uppercase tracking-wider transition-all duration-200 hover:opacity-90 active:scale-95 ${className}`}
      style={{
